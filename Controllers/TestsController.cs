...
//using ClosedXML.Excel;                // .xlsx with images
//using ClosedXML.Excel.Drawings;
//using ExcelDataReader;                 // legacy .xls (no images)
//using Microsoft.AspNetCore.Authorization;
//using Microsoft.AspNetCore.Hosting;    // for saving images
//using Microsoft.AspNetCore.Http;
//using Microsoft.AspNetCore.Mvc;
//using Microsoft.EntityFrameworkCore;
//using quizTool.Models;
...
//    {
//        private readonly QuizTool_Dbcontext _db;
//        private readonly IWebHostEnvironment _env; // CHANGED
...
//            // Map each answer with context
//            var items = new List<AttemptAnswerDto>();
//            foreach (var a in attempt.Answers.OrderBy(x => x.Id))
//            {
//                var q = test.Questions.FirstOrDefault(x => x.Id == a.QuestionId);
//                if (q == null) continue;

//                string? selectedText = null;
//                if (a.SelectedOptionId.HasValue)
//                {
//                    selectedText = q.Options.FirstOrDefault(o => o.Id == a.SelectedOptionId.Value)?.Text;
//                }

//                items.Add(new AttemptAnswerDto
//                {
//                    AnswerId = a.Id,
...
//            return Ok(new AttemptDetailDto
//            {
//                AttemptId = attempt.Id,
//                TestId = attempt.TestId,
...
//        // ---------- Update/override attempt score (admin - NEW) ----------
//        [Authorize(Roles = "admin")]
//        [HttpPatch("attempts/{attemptId:int}/score")]
//        public async Task<IActionResult> UpdateAttemptScore(int attemptId, [FromBody] UpdateAttemptScoreDto dto)
//        {
//            var attempt = await _db.TestAttempts
//                .Include(a => a.Answers)
//                .FirstOrDefaultAsync(a => a.Id == attemptId);
...
//            // Optionally mark specific answers as correct (e.g., subjective ones after review)
//            if (dto.MarkCorrectAnswerIds != null && dto.MarkCorrectAnswerIds.Any())
//            {
//                var set = dto.MarkCorrectAnswerIds.ToHashSet();
//                foreach (var ans in attempt.Answers)
//                {
//                    if (set.Contains(ans.Id))
//                        ans.IsCorrect = true;
//                }
//            }

//            // Validate and set final score
//            // Max allowed = total questions in its test (objective + subjective)
//            var max = await _db.Questions.CountAsync(q => q.TestId == attempt.TestId);
//            var newScore = Math.Clamp(dto.Score, 0, Math.Max(0, max));
//            attempt.Score = newScore;
...
//        // ---------- Get a single test (user-safe; never leaks IsCorrect) ----------
//        [HttpGet("{id:int}")]
//        public async Task<ActionResult<TestDetailDto>> GetTest(int id)
//        {
//            var test = await _db.Tests
...
//        // ---------- Admin test preview with answers (NEW) ----------
//        [Authorize(Roles = "admin")]
//        [HttpGet("{id:int}/admin")]
//        public async Task<ActionResult<AdminTestDetailDto>> GetTestAdmin(int id)
//        {
...
//                        ModelAnswer = q.ModelAnswer,
//                        Options = q.Type == QuestionType.Objective
//                            ? q.Options.Select(o => new AdminOptionFullDto
//                            {
//                                Id = o.Id,
//                                Text = o.Text,
//                                IsCorrect = o.IsCorrect
//                            }).ToList()
//                            : new List<AdminOptionFullDto>()
//                    })
//                    .ToList()
//            };
...
//        // ---------- Delete question (admin - NEW) ----------
//        [Authorize(Roles = "admin")]
//        [HttpDelete("questions/{questionId:int}")]
//        public async Task<IActionResult> DeleteQuestion(int questionId)
//        {
//            // remove attempt answers referencing this question
//            var answers = await _db.TestAttemptAnswers
//                                   .Where(a => a.QuestionId == questionId)
//                                   .ToListAsync();
//            if (answers.Count > 0)
//                _db.TestAttemptAnswers.RemoveRange(answers);
...
//            int totalObjective = test.Questions.Count(q => q.Type == QuestionType.Objective);
//            int score = 0;
